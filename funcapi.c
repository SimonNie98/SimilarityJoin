...
Datum jaccard_index(PG_FUNCTION_ARGS)
{
	...
	if(!m && !n)
		...
		//scan a;
		//head and tail
		h['$' - 32][tolower(a[0]) - 32] = 1;
		h[tolower(a[m-1]) - 32]['$' - 32] = 1;
		unions += 2;
		for(i = 0; i < m-1; i++){
			//not found in hash bucket
			int a1 = tolower(a[i]) - 32; 
         	int a2 = tolower(a[i+1]) - 32;
			if(!h[a1][a2]){
				h[a1][a2] = 1;
				unions++;
			}
		}
		if(h['$' - 32][tolower(b[0]) - 32] == 1)
			inter++;
		else unions++;
		
		if(h[tolower(b[n-1]) - 32]['$' - 32] == 1)
			inter++;
		else unions++;
		for(j = 0; j < n-1; j++){
			int b1 = tolower(b[j]) - 32;
			int b2 = tolower(b[j+1]) - 32;
			if(h[b1][b2] == 0){
